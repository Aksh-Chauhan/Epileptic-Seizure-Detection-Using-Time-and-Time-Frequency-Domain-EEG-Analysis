...
# Function to load and preprocess data for Case 2
def load_data(base_path):
    data = []
    labels = []
    label_mapping = {'Z': 0, 'O': 1, 'N': 1, 'F': 2, 'S': 2}  # Define label mapping for Case 2
...
X_train_time, X_val_time, X_train_tf, X_val_tf, y_train, y_val = train_test_split(
...
# Define the model architecture for Case 2
def create_model(input_shape_time, input_shape_tf):
    ...
    output = Dense(3, activation='softmax')(dense1)  # Adjust output layer for multi-class classification
...
# Create and train the model for Case 2
model = create_model(input_shape_time, input_shape_tf)
...
dummy_input_time = np.random.rand(1, normalized_data.shape[1], 1)
dummy_input_tf = np.random.rand(1, stft_data.shape[1], stft_data.shape[2], 1)
predicted_output = model.predict([dummy_input_time, dummy_input_tf])
print(f"Predicted Output for Case 2: {predicted_output}")

# Plot training history for accuracy
plt.figure(figsize=(10, 6))
plt.plot(history.history['accuracy'], label='Train Accuracy')
plt.plot(history.history['val_accuracy'], label='Val Accuracy')
plt.title('Model Training Accuracy for Case 2')
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.legend()
plt.show()
...
plt.title('Model Training vs Validation Loss for Case 2')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.show()

# Calculate F1 score, sensitivity, AUC for Case 2
y_pred = model.predict([X_val_time, X_val_tf])
y_pred_classes = np.argmax(y_pred, axis=-1)
f1 = f1_score(y_val, y_pred_classes, average='weighted')
conf_mat = confusion_matrix(y_val, y_pred_classes)
print(f"\nF1 Score for Case 2: {f1:.4f}")
